...
def db_select(query_str: str) -> Tuple[int, List[Tuple]]:
    conn = db_get_conn()
    cur = conn.cursor()
...
def search_poem(query_str: str = None, query_type: str = "title", items_per_page: int = 100, curr_page: int = 1) -> dict:
    ...
    for chars in str_choice:
        ...
        for poem in res_s[1]:
            if poem[0] in added_ids:
                pass
            else:
                res['result'].append(poem)
                res['num_res'] += 1
                added_ids.add(poem[0])
    return res


def query_poem_by_id(p_id: int) -> Tuple:
...
def search_author(query_str: str = None, items_per_page: int = 100, curr_page: int = 1) -> dict:
    ...
    for chars in str_choice:
        ...
        res_s = db_select(query_s)
        for item in res_s[1]:
            if item[0] in added_ids:
                pass
            else:
                res['result'].append(item)
                res['num_res'] += 1
                added_ids.add(item[0])
    return res


def query_poem_by_author(a_id: int, items_per_page: int = 100, curr_page: int = 1) -> List[Tuple]:
    query_s = f"SELECT p.p_id, p.p_title, a.a_id, a.a_name, r.r_id, r.r_name, p.p_paragraph " \
              f"FROM poetry as p " \
              f" LEFT JOIN author as a ON p.p_author_id = a.a_id " \
              f" LEFT JOIN rhythmic as r ON p.p_rhythmic_id = r.r_id " \
...
def display_rhythmic(items_per_page: int = 100, curr_page: int = 1) -> List[Tuple]:
...
def query_poem_by_rhythmic(r_id: int, items_per_page: int = 100, curr_page: int = 1) -> List[Tuple]:
    query_s = f"SELECT p.p_id, p.p_title, a.a_id, a.a_name, r.r_id, r.r_name, p.p_paragraph " \
              f"FROM poetry as p " \
              f" LEFT JOIN author as a ON p.p_author_id = a.a_id " \
              f" LEFT JOIN rhythmic as r ON p.p_rhythmic_id = r.r_id " \
...
def display_collection() -> List[Tuple]:
    query_s = f"SELECT c_id, c_name, c_note FROM collection"
    res_s = db_select(query_s)
    return res_s[1]


def query_poem_by_collection(c_id: int, items_per_page: int = 100, curr_page: int = 1) -> List[Tuple]:
    query_s = f"SELECT p.p_id, p.p_title, a.a_id, a.a_name, r.r_id, r.r_name, p.p_paragraph " \
              f"FROM poetry as p " \
              f" LEFT JOIN author as a ON p.p_author_id = a.a_id " \
              f" LEFT JOIN rhythmic as r ON p.p_rhythmic_id = r.r_id " \
