<template>
        ...
        <n-card title="" size="large" v-if="this.ready_to_render">
                ...
                <n-button circle style="margin-left: 10px; margin-top: 5px;" @click="randomize">
                    <template #icon>
                        <n-icon><SyncCircle /></n-icon>
                    </template>
                </n-button>
                <n-h4>{{ poem_info_.a_name }} &nbsp {{ poem_info_.d_name }}</n-h4>
                <n-h2 v-html="poem_info_.p_paragraph">
                </n-h2>

                <n-blockquote style="margin-top: 3rem;">
                    {{ poem_info_.p_note }}
                </n-blockquote>
...
<script>
    ...
    import { NCard, NH2, NP, NH4, NH1, NBlockquote, NButton, NIcon, NGrid, NGi, NImage } from 'naive-ui';
    import { SyncCircleOutline as SyncCircle } from '@vicons/ionicons5'
    ...
    export default defineComponent({
        components: {
            NCard, NH2, NP, NH4, NBlockquote, NH1, NButton, SyncCircle, NIcon, NGrid, NGi, NImage
        },
        methods: {
            ...
            clean_data: function () {
                this.poem_info_.p_paragraph = this.poem_info_.p_paragraph.replace(/(?:\r\n|\r|\n)/g, '<br>')
                if (this.poem_info_.p_title === null || this.poem_info_.p_title === '')
                {
                    if (this.poem_info_.r_name === null || this.poem_info_.r_name === '')
                    {
                        this.poem_title = '无题'
                    }else {
                        this.poem_title = this.poem_info_.r_name
                    }
                }else {
                    this.poem_title = this.poem_info_.p_title
                }
                if (this.poem_info_.a_name === null || this.poem_info_.a_name === '')
                {
                    this.poem_info_.a_name = '无名'
                }
                if (this.poem_info_.p_note === null || this.poem_info_.p_note === '')
                {
                    this.poem_info_.p_note = ''
                }
                if (this.poem_info_.r_name === null || this.poem_info_.r_name === '')
                {
                    this.has_rhythmic = false
                }else {
                    this.has_rhythmic = true
                }
                this.poem_title = this.simplized(this.poem_title)
                this.poem_info_.p_paragraph = this.simplized(this.poem_info_.p_paragraph)
                this.poem_info_.a_name = this.simplized(this.poem_info_.a_name)
                this.poem_info_.d_name = this.simplized(this.poem_info_.d_name)
                this.poem_info_.p_note = this.simplized(this.poem_info_.p_note)
            },
            trad_simp_conv : function () {
                if (this.simplified) {
                    this.poem_title = this.traditionalized(this.poem_title)
                    this.poem_info_.p_paragraph = this.traditionalized(this.poem_info_.p_paragraph)
                    this.poem_info_.a_name = this.traditionalized(this.poem_info_.a_name)
                    this.poem_info_.d_name = this.traditionalized(this.poem_info_.d_name)
                    this.poem_info_.p_note = this.traditionalized(this.poem_info_.p_note)
            ...
            randomize: function () {
                axios.get( this.BASE_URL + "/query/random_poem")
                        .then((response) => {
